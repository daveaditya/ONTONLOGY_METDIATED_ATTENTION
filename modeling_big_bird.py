...
class BigBirdBlockSparseAttention(nn.Module):
    def __init__(self, config, seed=None):
        ...
        self.key = nn.Linear(config.hidden_size, self.all_head_size, bias=config.use_bias)
        self.value = nn.Linear(config.hidden_size, self.all_head_size, bias=config.use_bias)
        self.selective_window=config.selective_window
        self.block_selective_attn=config.block_selective_attn
        self.block_global_attn=config.block_global_attn
    ...
    def forward(
        ...
        context_layer, attention_probs = self.bigbird_block_sparse_attention(
            ...
            from_seq_length,
            to_seq_length,
            selective_attention_mask,
            self.selective_window,
            self.block_selective_attn,
            self.block_global_attn,
            seed=self.seed,
    ...
    def get_selective_values(self, selective_attention_mask, key, value, window= 1):
        rand_indices = []
        batch_size, num_heads, seq_len, hidden_dim = key.size() 
        print(selective_attention_mask)
        for bs in range(batch_size):
            rands = torch.randint(low=0, high=len(selective_attention_mask[bs]), size=(int(num_heads* seq_len/window),))
            chosen_atts = torch.nn.functional.relu(selective_attention_mask[bs][rands]-window//2)
            chosen_atts = chosen_atts - torch.nn.functional.relu(chosen_atts-(seq_len-window))
            rand_windows = [torch.arange(val,val+window) for val in chosen_atts]
            rand_indices.append(torch.cat(rand_windows))
        rand_indices = torch.stack(rand_indices)
        
        # Select elements from the key matrix each batch and each head using the randomly selected indices
        head_indexer = torch.repeat_interleave(torch.arange(num_heads), seq_len)
        select_keys = [k_mat[head_indexer,r_mat] for k_mat, r_mat in zip(key, rand_indices)]
        select_keys = torch.stack(select_keys)
        select_keys = select_keys.reshape(batch_size, num_heads, seq_len, hidden_dim)
        select_value = [k_mat[head_indexer,r_mat] for k_mat, r_mat in zip(value, rand_indices)]
        select_value = torch.stack(select_value)
        select_value = select_value.reshape(batch_size, num_heads, seq_len, hidden_dim)
        return select_keys, select_value
    ...
    def bigbird_block_sparse_attention(
        ...
        #ADD WINDOW TO THE FORWARD PASS PATH LATER
        selective_window = 4
        block_selective_attn = True
        block_global_attn = True
        ...
        selective_key_layer, selective_value_layer = self.get_selective_values(selective_attention_mask, key_layer, value_layer, selective_window)
        selective_blocked_key_matrix = selective_key_layer.view(bsz, n_heads, to_seq_len // to_block_size, to_block_size, -1)
        selective_blocked_value_matrix = selective_value_layer.view(bsz, n_heads, to_seq_len // to_block_size, to_block_size, -1)
        #blocked_value_matrix_selected = # some permutation of blocked_value_matrix
        # preparing block for randn attn
        if block_selective_attn:
            gathered_key = self.torch_gather_b2(selective_blocked_key_matrix, rand_attn)
            gathered_key = gathered_key.view(
                bsz, n_heads, to_seq_len // to_block_size - 2, n_rand_blocks * to_block_size, -1
            )  # [bsz, n_heads, to_seq_len//to_block_size-2, n_rand_blocks, to_block_size, -1]
            gathered_value = self.torch_gather_b2(selective_blocked_value_matrix, rand_attn)
        ...
        global_attn_key_block_end = blocked_key_matrix[:, :, -1]
        global_attn_value_block_end = blocked_value_matrix[:, :, -1]
        if block_global_attn:
            # MIGHT BE CHANGED IF WE MAKE SMARTER SELECTION
            global_attn_key_block_end = selective_blocked_key_matrix[:, :, -1]
            global_attn_value_block_end = selective_blocked_value_matrix[:, :, -1]
        # 1st PART
        ...
        last_band_product = torch.einsum(
            "bhlqd,bhkd->bhlqk", middle_query_matrix, global_attn_key_block_end
        )  # [bsz, n_heads, from_seq_len//from_block_size-4, from_block_size, -1] x [bsz, n_heads, to_block_size, -1] ==> [bsz, n_heads, from_seq_len//from_block_size-4, from_block_size, to_block_size]
        ...
        context_layer += torch.einsum(
            "bhlqk,bhkd->bhlqd", attn_weights[:, :, :, :, -to_block_size:], global_attn_value_block_end
        )  # [bsz, n_heads, from_seq_len//from_block_size-4, from_block_size, to_block_size] x [bsz, n_heads, to_block_size, -1] ==> [bsz, n_heads, from_seq_len//from_block_size-4, from_block_size, -1]
...
class BigBirdAttention(nn.Module):
    ...
    def forward(
        ...
        print('big bird attention {}'.format(selective_attention_mask))
        if self.attention_type == "original_full":
            ...
            self_outputs = self.self(
                hidden_states, band_mask, from_mask, to_mask, from_blocked_mask, to_blocked_mask, output_attentions, selective_attention_mask
            )
...
class BigBirdLayer(nn.Module):
    ...
    def forward(
        ...
        print('big bird layer {}'.format(selective_attention_mask))
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                print('big bird encoder {}'.format(selective_attention_mask))
...
class BigBirdModel(BigBirdPreTrainedModel):
    ...
    def forward(
        ...
        print('big bird model {}'.format(selective_attention_mask))
        encoder_outputs = self.encoder(
            embedding_output,
            attention_mask=extended_attention_mask,
            selective_attention_mask=selective_attention_mask,
            head_mask=head_mask,
            encoder_hidden_states=encoder_hidden_states,
